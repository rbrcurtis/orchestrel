/* oxlint-disable orchestrel/log-before-early-return -- pure message/tool conversion, guard returns without session context */
/*
 * Claude Code request reshaping — message + tool conversion.
 *
 * Vendored/adapted from leohenon/pi-anthropic-oauth (MIT). Converts Pi's
 * in-memory message/tool model into Anthropic Messages params, mapping tool
 * names to their Claude Code spellings (read → Read) so the request matches what
 * the classifier expects, and repairing orphan tool_use / tool_result pairs that
 * would otherwise make Anthropic reject the next request.
 */
...
export function convertPiMessagesToAnthropic(messages: Message[]): MessageParam[] {
  ...
  // Anthropic requires every tool_use block to be IMMEDIATELY followed by a user
  // message with a matching tool_result. Aborted/errored turns or partial
  // histories can leave orphan tool_uses, which makes the API reject the next
  // request. Track outstanding ids and synthesize "No result provided" results.
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (message.role === 'assistant') {
      // Skip aborted/errored assistant turns entirely — partial tool_use blocks
      // would never get a matching tool_result and poison subsequent requests.
    ...
    if (message.role === 'toolResult') {
      ...
      // Cover any tool_use from the last assistant message not satisfied by this
      // run. They must live in the SAME user message as the real results.
      for (const id of pendingToolUseIds) {
        if (!satisfiedIds.has(id)) {
          toolResults.push({ type: 'tool_result', tool_use_id: id, content: 'No result provided', is_error: true });
        }
      }
      pendingToolUseIds = [];
...
export function convertPiToolsToAnthropic(tools: Tool[]): ToolUnion[] {
