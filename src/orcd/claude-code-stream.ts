/* oxlint-disable orchestrel/log-before-early-return, orchestrel/log-in-catch -- pure SDK stream boundary; partial-JSON catches are intentional no-ops and the error path reshapes into a stream error event */
/*
    ...
    void (async () => {
      ...
      try {
        ...
        const params: MessageCreateParamsStreaming = {
          model: model.id,
          messages: convertPiMessagesToAnthropic(context.messages),
          max_tokens: maxTokens,
          stream: true,
        };

        const system = buildClaudeCodeSystemPrompt(context.systemPrompt);
        if (system) params.system = system as never;
        if (context.tools?.length) params.tools = convertPiToolsToAnthropic(context.tools);
        ...
        if (options?.reasoning && model.reasoning && maxTokens > 1) {
          ...
          const requestedBudget = customBudget ?? defaultBudgets[options.reasoning] ?? 10240;
          params.thinking = { type: 'enabled', budget_tokens: Math.min(requestedBudget, maxTokens - 1) };
        }
        ...
        for await (const event of anthropicStream) {
          if (event.type === 'message_start') {
            ...
            output.usage.cacheWrite =
              (event.message.usage as { cache_creation_input_tokens?: number }).cache_creation_input_tokens || 0;
          ...
          if (event.type === 'content_block_start') {
            if (event.content_block.type === 'text') {
              output.content.push({ type: 'text', text: '', index: event.index } as IndexedBlock);
              ...
              output.content.push({
                ...
                name: fromClaudeCodeToolName(event.content_block.name, context.tools),
                arguments: {},
                partialJson: '',
                index: event.index,
              } as IndexedBlock);
          ...
          if (event.type === 'message_delta') {
            ...
            output.usage.input = (event.usage as { input_tokens?: number }).input_tokens || output.usage.input;
            output.usage.output = (event.usage as { output_tokens?: number }).output_tokens || output.usage.output;
            output.usage.cacheRead =
              (event.usage as { cache_read_input_tokens?: number }).cache_read_input_tokens || 0;
            output.usage.cacheWrite =
              (event.usage as { cache_creation_input_tokens?: number }).cache_creation_input_tokens || 0;
            output.usage.totalTokens =
              output.usage.input + output.usage.output + output.usage.cacheRead + output.usage.cacheWrite;
            calculateCost(model, output.usage);
          }
        ...
        for (const block of output.content as Array<{ index?: number; partialJson?: string }>) {
          delete block.index;
