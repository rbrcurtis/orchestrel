...
function toCardResponse(card: Card): CardResponse {
  ...
  const status = !client?.isConnected() ? 'unavailable' : active ? 'running' : starting ? 'starting' : 'completed';
  ...
  return {
    ...
    sleepPrompt: card.sleepPrompt ?? null,
...
export class CardsController extends Controller {
  ...
  public async listCards(
    ...
    if (term) {
      base.andWhere('(instr(lower(card.title), :search) > 0 OR instr(lower(card.description), :search) > 0)', {
        search: term,
      });
    }
