...
describe('CardService', () => {
  ...
  it('archiveOthers only archives active cards in the same project', async () => {
    ...
    const sameProject = await cardService.createCard({
      title: 'Same project',
      description: 'd',
      column: 'ready',
      projectId: projectA.id,
    });
    const otherProject = await cardService.createCard({
      title: 'Other project',
      description: 'd',
      column: 'ready',
    ...
    const alreadyDone = await cardService.createCard({
      title: 'Already done',
      description: 'd',
      column: 'done',
      projectId: projectA.id,
  ...
  it('applies project worktree and base-branch defaults, respecting an explicit opt-out', async () => {
    ...
    const noWt = await cardService.createCard({
      title: 'Plain card',
      description: 'd',
      column: 'backlog',
      projectId: proj.id,
  ...
  it('inherits the project summarize default when creating a card, unless an explicit value is given', async () => {
    ...
    const offCard = await cardService.createCard({
      title: 'Off card',
      description: 'd',
      column: 'backlog',
      projectId: plain.id,
  ...
  it('rejects relative import paths before calling a node', async () => {
    ...
    await expect(
      cardService.importSession({ sessionId: 'relative-path', path: 'relative/path', nodeName: 'local' }),
  ...
  it('imports a session into its exact project using the first substantive user message', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: 'Import endpoint' }), { status: 200 }));

    const card = await cardService.importSession({
  ...
  it('does not import a same-path project configured on another node', async () => {
    ...
    await projectService.createProject({
      name: 'Other node project',
      path: '/tmp/import-other-node',
      nodeName: 'other',
    });
    ...
    await expect(
      cardService.importSession({ sessionId: 'other-node-session', path: '/tmp/import-other-node', nodeName: 'local' }),
  ...
  it('imports a session from a configured project linked worktree using that worktree history', async () => {
    ...
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: 'Linked worktree import' }), { status: 200 }));
  ...
  it('rejects importing a session from an unrelated git worktree', async () => {
    ...
    await expect(
      cardService.importSession({ sessionId: 'no-project', path: '/tmp/unrelated-worktree', nodeName: 'local' }),
    ).rejects.toThrow('No project configured for path');
  });
  ...
  it('rejects duplicate sessions before reading history', async () => {
    ...
    await expect(
      cardService.importSession({ sessionId: 'duplicate-session', path: '/tmp/import-exact', nodeName: 'local' }),
