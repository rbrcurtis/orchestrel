...
export function setIo(instance: AppServer): void {
  state.io = instance;
}

/** Cached event bus — publishers and listeners must share it across Vite bundles. */
export function getMessageBus(): EventEmitter | null {
  return state.messageBus;
}
export function setMessageBus(bus: EventEmitter): void {
  state.messageBus = bus;
}
