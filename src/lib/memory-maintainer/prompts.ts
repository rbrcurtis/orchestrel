...
- DELETE only when an existing memory is stale AND a remaining memory covers the same specific facts. Before deleting, search for the replacement and verify it exists and matches. If supersession is not demonstrable, SKIP. Deleting is permanent; leaving a stale memory is safer than deleting a live one.
